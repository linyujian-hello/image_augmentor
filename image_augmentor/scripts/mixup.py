import argparse, os
from auglib import list_images, load_image, save_image, ensure_rgb, resize_image, mixup_pair
...
def main(input_dir, output_dir, alpha, out_size):
    ...
    for i in range(0, n, 2):
        if i+1 >= n:
            ...
            save_image(os.path.join(output_dir, f"{base}_mixup_single.jpg"), img)
            continue
        ...
        mixed, (l1, l2, lam) = mixup_pair(img1, None, img2, None, alpha=alpha)
        base1 = os.path.splitext(os.path.basename(files[i]))[0]
        base2 = os.path.splitext(os.path.basename(files[i+1]))[0]
        save_image(os.path.join(output_dir, f"{base1}_{base2}_mixup_{lam:.3f}.jpg"), mixed)
